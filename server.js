...
function generatePDF(htmlContent) {
    return new Promise((resolve, reject) => {
        htmlToPdf.create(htmlContent, { width: '4in' , height: '4.5in', timeout: '300000' }).toBuffer((err, buffer) => {
            if (err) {
                reject(err);
            } else {
                resolve(buffer);
...
async function generateHtmlContent(order) {
    ...
    <body>
        <div class="content">
            ...
            <table>
                <tr>
                    <td style="font-size: 20px;padding: 8px;text-align:center;">TO</td>
                    <td>
...
// MongoDB connection
const dbURI = `mongodb+srv://${mongoUser}:${mongoPassword}@cluster0.pbjj6kp.mongodb.net/F3-DB?retryWrites=true&w=majority&appName=Cluster0`;
...
// POST route for storing payment details
...
app.get('/getPayments', async (req, res) => {
  try {
    const savedPayment = await Payment.find({}).sort({ createdAt: -1 }).exec();
    res.status(201).send(savedPayment);
  } catch (error) {
    res.status(400).send(error);
  }
